...
BEGIN
    ...
    --                 The ID should be an integer, while name, surname,
    --                 and date of birth are provided with specific formats.
    --
    -- Returns:
    ...
    --         - DOB (DATE): The author's date of birth in a date format.
    --
    -- Example Call: 
    --     SELECT * FROM dbo.SplitBookInfo('1,''John'',''Doe'',''03/01/1986''');
    --
    ...
    SELECT 
        ...
        CAST(MAX(CASE WHEN RowNum = 4 THEN value END) AS DATE)  -- Cast fourth value to date of birth (DOB)
    FROM SplitValues;
